...
\section{Pilot Experiments and Current Implementation Status}\label{sec:pilot}
...
\begin{table}[h]
\caption{Pilot TFT experiments completed before the full multi-city station-level run. Lower MAE and RMSE indicate better absolute forecasting accuracy.}\label{tab:pilot_models}
\begin{tabular}{@{}lllll@{}}
\toprule
Experiment & Scope & Main inputs & MAE & RMSE\\
\midrule
Daily TFT & Almaty city-level & Daily PM$_{2.5}$ and pollutant covariates & 20.27 & 25.03\\
Daily TFT + weather & Almaty city-level & Daily PM$_{2.5}$, pollutants, weather & 19.18 & 22.73\\
Hourly TFT & Almaty city-level & Hourly PM$_{2.5}$, lags, rolling features & 16.60 & 20.87\\
Hourly TFT + weather & Almaty city-level & Hourly PM$_{2.5}$, weather, temporal features & 15.02 & 19.61\\
\botrule
\end{tabular}
\end{table}

The pilot results suggest two preliminary findings. First, moving from daily to hourly data improved absolute forecasting performance because the model could capture intraday variation, lagged hourly dependence, and weekly cycles. Second, adding weather variables improved MAE and RMSE in the Almaty hourly experiment, although MAPE remained high because percentage error is unstable when PM$_{2.5}$ concentrations are low. Therefore, the main comparison should emphasize MAE and RMSE for concentration forecasting, while health-risk evaluation should emphasize recall and F1-score for unhealthy-or-worse events.
...
Several results are expected from the proposed study. First, TFT is expected to outperform simple persistence and rolling-average baselines and remain competitive with other deep learning approaches because of its ability to represent nonlinear temporal dependencies and heterogeneous covariates. Second, the station-level multi-city architecture is expected to improve coverage and scalability compared with a single aggregated city-level model, which is important for Kazakhstan-wide environmental analytics.
