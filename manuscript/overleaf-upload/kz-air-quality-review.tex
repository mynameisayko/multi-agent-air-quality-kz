...
The health risk module transforms both observed and predicted PM$_{2.5}$ concentrations into categorical warning labels. Separate historical health-label datasets are not required for this first stage because the labels are deterministically derived from pollutant concentrations. The current implementation uses four risk categories: safe, moderate, unhealthy, and hazardous. The thresholds are defined as follows: safe for PM$_{2.5}$ up to 12.0 $\mu$g/m$^3$, moderate for 12.1--35.4 $\mu$g/m$^3$, unhealthy for 35.5--150.4 $\mu$g/m$^3$, and hazardous above 150.4 $\mu$g/m$^3$. These categories are used as risk-warning proxies rather than clinical diagnoses.
...
Model performance should be evaluated with error metrics appropriate for regression forecasting, such as MAE, RMSE, and MAPE. MAE is used as the most directly interpretable metric because it reports the average absolute error in $\mu$g/m$^3$. RMSE is used because it penalizes large errors more strongly and is therefore sensitive to missed pollution peaks. MAPE is reported for comparability, but it should not be treated as the primary metric for PM$_{2.5}$ because percentage errors become unstable when observed concentrations are low.
...
The pilot results suggest three preliminary findings. First, moving from daily to hourly data improved absolute forecasting performance because the model could capture intraday variation, lagged hourly dependence, and weekly cycles. Second, adding weather variables improved MAE and RMSE in the Almaty hourly experiment, although MAPE remained high because percentage error is unstable when PM$_{2.5}$ concentrations are low. Third, the station-level multi-city formulation produced the best current MAE and RMSE, suggesting that learning across multiple monitoring stations can improve the representation of urban PM$_{2.5}$ dynamics compared with a single city-level aggregate.
...
MAPE & Regression & Percentage error; reported for comparison but unstable at low PM$_{2.5}$\\
...
\caption{Comparison between TFT and simple baseline models on the imported Kaggle forecast output.}
...
The station-level multi-city model further improved absolute-error metrics compared with the Almaty city-level pilots. This suggests that a shared model can benefit from learning across multiple monitoring stations while still preserving local differences through station and city identifiers. The result supports the methodological choice to model each station as a separate time series rather than reducing each city to a single average concentration. However, the imported baseline comparison also shows that simple persistence and rolling-mean baselines currently outperform the TFT checkpoint in MAE and RMSE on the short test window. Therefore, the current result should be interpreted as a preliminary implementation milestone rather than a final claim of model superiority.

MAPE remained comparatively high across hourly experiments. This does not necessarily mean that the model is unusable; rather, MAPE is sensitive to low observed PM$_{2.5}$ values. When actual concentrations are small, even a small absolute error can become a large percentage error. For this reason, MAE and RMSE are more reliable primary metrics for concentration forecasting, while risk-oriented metrics are more appropriate for evaluating warning performance.
